						...
						for( tile_n = 0; tile_n < tiles_cnt; tile_n++ )
						{
							...
							for( block_n = 0; block_n < utils.CONST_BLOCK_SIZE; block_n++ )
							{
...
#endif									
								{
									...
									{
										block_id = exp_scr.m_scr_blocks.get_tile( ( ( tile_n / utils.CONST_SCREEN_NUM_HEIGHT_TILES ) * ( scr_height_blocks << 1 ) ) + tile_offs_y + ( block_n >> 1 ) + ( ( block_n & 0x01 ) * scr_height_blocks ) );
									}
						...
						if( scr_data.m_scr_ind != layout_data.CONST_EMPTY_CELL_ID )
						{
							...
							if( screens.ContainsKey( scr_key ) )
							{
								...
								if( enable_comments )
								{
									...
									if( RBtnLayoutMatrix.Checked )
									{
										level_data.export_asm( null, _sw, CONST_FILENAME_LEVEL_PREFIX + level_n, ".define", ".byte", ".word", ".word", "$", false, false, false, false );
									}
								}
		...
		private void save_multidir_scroll_mode( StreamWriter _sw )
		{
			...
			for( int level_n = 0; level_n < n_levels; level_n++ )
			{
				...
				else
				{
					...
					if( RBtnTilesDirRows.Checked )
					{
						utils.swap_columns_rows_order<byte>( map_tiles_arr, n_scr_X * utils.CONST_SCREEN_NUM_WIDTH_TILES, n_scr_Y * utils.CONST_SCREEN_NUM_HEIGHT_TILES );
					}
				...
				if( CheckBoxExportEntities.Checked )
				{
					level_data.export_asm( null, _sw, CONST_FILENAME_LEVEL_PREFIX + level_n, ".define", ".byte", ".word", ".word", "$", true, CheckBoxExportMarks.Checked, CheckBoxExportEntities.Checked, RBtnEntityCoordScreen.Checked );
				}
				else
				{
