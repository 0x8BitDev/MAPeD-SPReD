...
namespace MAPeD
{
	...
	public partial class exporter_sms_asm : Form
	{
#if DEF_SMS
		...
		public exporter_sms_asm( data_sets_manager _data_mngr )
		{
		...
		{
			ushort block_data = _tiles.blocks[ ( _block_n << 2 ) + _chr_n ];
			
			return get_screen_attribute( block_data );
		}
		
		private ushort get_screen_attribute( ushort _block_data )
		{
			...
			// add flipping
			{
				res |= ( ushort )( tiles_data.get_block_flags_flip( _block_data ) << 10 );
			}
		...
		private void save_multidir_scroll_mode( StreamWriter _sw )
		{
			...
			for( int level_n = 0; level_n < n_levels; level_n++ )
			{
				...
				// tiles 2x2
				{
					if( RBtnTiles2x2.Checked )
					{
						max_block_ind = 1 + utils.get_byte_arr_max_val( map_blocks_arr );	// one based index	
					}
					else
					{
					...
					_sw.WriteLine( label + ":\t.incbin \"" + m_filename + "_" + label + CONST_BIN_EXT + "\"\t; " + "(" + data_size + ") blocks screen attributes array ( 2 bytes per attribute )" );
				}
