...
//
//	 The case, when your meta-sprite has more than one palette,
//	 you need to export each sprite set with its own unique
//	 palettes. But all exported sprite sets will share the same
//	 graphics.
//
...
// Data export steps:
// ~~~~~~~~~~~~~~~~~~
// Delete the 'dummy_sprite_for_palettes'.
//
// VADDR $2000
//
// 1. Exported name: chin
//    Comment CHR data: X
//    Add filename to sprite names: V
//    Data directory: data
//
// 2. Apply palette #01 to all sprites
//    Exported name: chin_pink
//    Comment CHR data: V
//    Add filename to sprite names: V
//    Data directory: data
//
// 3. Apply palette #02 to all sprites
//    Exported name: chin_blue
//    Comment CHR data: V
//    Add filename to sprite names: V
//    Data directory: data
//
...
void	push_sprite_chin_violet( sprite_desc* _desc )
{
...
void	push_sprite_chin_blue( sprite_desc* _desc )
{
#if	DEF_SG_DBL_BUFF
	spd_sprite_params( chin_SG_arr, CHIN_BLUE_SPR_VADDR, SPD_FLAG_DBL_BUFF, _desc->last_bank );
	spd_dbl_buff_VRAM_addr( _desc->dbf_VADDR, _desc->dbf_ind );
#else
	spd_sprite_params( chin_SG_arr, CHIN_BLUE_SPR_VADDR, SPD_FLAG_PEND_SG_DATA, _desc->last_bank );
	spd_SG_data_params( &_desc->SG_src_addr, &_desc->SG_src_bank, &_desc->SG_dst_addr, &_desc->SG_data_len );
#endif
	spd_alt_VRAM_addr( _desc->alt_VADDR );

	_desc->push_spr_res	= spd_SATB_push_sprite( chin_blue_frames_data, _desc->curr_frame, _desc->x, _desc->y );
	_desc->last_bank	= spd_SG_bank_get_ind();
...
void	push_sprite_chin_pink( sprite_desc* _desc )
{
#if	DEF_SG_DBL_BUFF
	spd_sprite_params( chin_SG_arr, CHIN_PINK_SPR_VADDR, SPD_FLAG_DBL_BUFF, _desc->last_bank );
	spd_dbl_buff_VRAM_addr( _desc->dbf_VADDR, _desc->dbf_ind );
#else
	spd_sprite_params( chin_SG_arr, CHIN_PINK_SPR_VADDR, SPD_FLAG_PEND_SG_DATA, _desc->last_bank );
	spd_SG_data_params( &_desc->SG_src_addr, &_desc->SG_src_bank, &_desc->SG_dst_addr, &_desc->SG_data_len );
#endif
	spd_alt_VRAM_addr( _desc->alt_VADDR );

	_desc->push_spr_res	= spd_SATB_push_sprite( chin_pink_frames_data, _desc->curr_frame, _desc->x, _desc->y );
	_desc->last_bank	= spd_SG_bank_get_ind();
...
/* all sprite actions initialization at startup */
void	init_actions()
{
	sprite_desc*	desc;
	char		i;
...
void	sprite_cache_update()
{
	...
	for( i = 0; i < DEF_SPR_CACHE_SIZE; i++ )
	{
		...
		if( desc->active )
		{
			...
			if( desc->spr_type == SPR_TYPE_CHIN_VIOLET )
			{
				push_sprite_chin_violet( desc );
			}
			else
			if( desc->spr_type == SPR_TYPE_CHIN_PINK )
			{
				push_sprite_chin_pink( desc );
			}
			else
			if( desc->spr_type == SPR_TYPE_CHIN_BLUE )
			{
				push_sprite_chin_blue( desc );
			}
		}
	}
}
...
.endif
	...
	load_palette( CHIN_PINK_PALETTE_SLOT, chin_pink_palette, CHIN_PINK_PALETTE_SIZE );
	load_palette( CHIN_BLUE_PALETTE_SLOT, chin_blue_palette, CHIN_BLUE_PALETTE_SIZE );
